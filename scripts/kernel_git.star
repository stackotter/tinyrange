REV = "6.10"
URL = "https://github.com/torvalds/linux/archive/refs/tags/v6.10.tar.gz"

kernel_tgz = define.read_archive(define.fetch_http(URL), ".tar.gz")
...
config = define.build_vm(
    directives = [
        builder_plan,
        kernel_tgz,
        directive.add_file("/root/.config", file(CONFIG)),
        directive.run_command("mv /linux-{} /root/linux".format(REV)),
        directive.run_command("mv /root/.config /root/linux"),
        directive.run_command("cd /root/linux;make olddefconfig"),
    ],
    output = "/root/linux/.config",
    **vm_settings
)

build = define.build_vm(
    directives = [
        builder_plan,
        kernel_tgz,
        directive.add_file("/root/.config", config),
        directive.run_command("mv /linux-{} /root/linux".format(REV)),
        directive.run_command("mv /root/.config /root/linux"),
        directive.run_command("export KBUILD_BUILD_TIMESTAMP='';cd /root/linux;make -j4"),
    ],
    output = "/root/linux/arch/arm64/boot/Image",
    **vm_settings
)

boot = define.build_vm(
    directives = [
        builder_plan,
